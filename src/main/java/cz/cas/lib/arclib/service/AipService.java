...
public class AipService {
    ...
    public ClientHttpResponse removeAip(String aipId) throws IOException, URISyntaxException {
        ...
        if (clientHttpResponse.getStatusCode().is2xxSuccessful()) {
            ingestWorkflowService.findBySipId(aipId).forEach(iw -> {
                solrArclibXmlStore.changeState(iw.getExternalId(), IndexedArclibXmlDocumentState.REMOVED);
    ...
    public ClientHttpResponse renewAip(String aipId) throws IOException, URISyntaxException {
        ...
        if (clientHttpResponse.getStatusCode().is2xxSuccessful()) {
            ingestWorkflowService.findBySipId(aipId).forEach(iw -> {
                solrArclibXmlStore.changeState(iw.getExternalId(), IndexedArclibXmlDocumentState.PERSISTED);
    ...
    private ClientHttpResponse deleteAip(String aipId) throws IOException, URISyntaxException {
        ...
        if (clientHttpResponse.getStatusCode().is2xxSuccessful()) {
            ingestWorkflowService.findBySipId(aipId).forEach(iw -> {
                solrArclibXmlStore.changeState(iw.getExternalId(), IndexedArclibXmlDocumentState.DELETED);
    ...
    public ResponseEntity<String> finishXmlUpdate(String aipId, String xmlId, String xml, Hash hash, Integer xmlVersion,
        ...
        if (archivalStorageResponse.getStatusCode().is2xxSuccessful()) {
            ...
            solrArclibXmlStore.createIndex(xml, producer, userDetails.getUser(), IndexedArclibXmlDocumentState.PERSISTED, originalIngestWorkflow.getBatch().isDebuggingModeActive());
