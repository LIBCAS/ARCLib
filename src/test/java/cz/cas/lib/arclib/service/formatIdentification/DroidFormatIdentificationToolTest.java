...
public class DroidFormatIdentificationToolTest {
    ...
    public void testThatAllFilesScanned() throws IOException {
        Map<String, List<Utils.Pair<String, String>>> result = formatIdentificationTool.analyze(SIP_PATH);
    ...
    public void analyzeRightExtensionsIdentifiedTest() throws IOException {
        ...
        assertThat(result.get(filePath1), contains(new Utils.Pair("fmt/101", "Signature")));

        String filePath2 = ("file://desktop.ini");
        assertThat(result.get(filePath2), contains(new Utils.Pair("x-fmt/421", "Extension")));

        String filePath3 = ("file://KPW01169310.md5");
        assertThat(result.get(filePath3), contains(new Utils.Pair("fmt/993", "Extension")));

        String filePath4 = ("file://TXT/TXT_KPW01169310_0002.TXT");
        assertThat(result.get(filePath4), contains(new Utils.Pair("x-fmt/111", "Extension")));

        String filePath5 = ("file://userCopy/UC_KPW01169310_0001.JP2");
        assertThat(result.get(filePath5), contains(new Utils.Pair("x-fmt/392", "Signature")));
    }
}
