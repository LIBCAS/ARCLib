...
public class ReportStore extends DomainStore<Report, QReport> {
     ...
     * @return
     */
    public String saveReport(String name, InputStream template, boolean arclibXmlDs) throws IOException {
        ...
        template = new ByteArrayInputStream(templateBytes);
        String id;
        id = save(new Report(name, IOUtils.toString(template, StandardCharsets.UTF_8), compiledReport, arclibXmlDs)).getId();
        return id;
    }
