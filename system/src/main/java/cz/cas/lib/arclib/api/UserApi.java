...
public class UserApi {
    ...
    @ApiOperation(value = "Gets all available roles that calling user can assign", responseContainer = "list", response = Role.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Successful response")})
    @RequestMapping(value = "/roles", method = RequestMethod.GET)
    @RolesAllowed({Roles.ADMIN, Roles.SUPER_ADMIN})
    public Collection<Role> getAssignableRoles() {
        Collection<Role> all = roleStore.findAll();
        if (!hasRole(userDetails, Roles.SUPER_ADMIN)) {
            all = all.stream().filter(r -> !r.getName().equals(Roles.SUPER_ADMIN)).collect(Collectors.toList());
        }
        return all;
    }
